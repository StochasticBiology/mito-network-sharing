...
// let's try and make this as general as possible
// bingo type 0 -- no gene decay, zero initial conditions, edge wiping (original)
// bingo type 1 -- slow gene decay, full initial conditions, edge wiping
// bingo type 2 -- fast gene decay, full initial conditions, edge wiping

void PlayBingoRNA(int *amlist, int amn, int max, int L, double exprop, int bingotype, double *total, double *bingo)
{
  int i, j, k;
  int r;
  double *gene, *dna;
  ...
  double decayrate;
  int thisovermin;
  int emptyic, geneexchange; 

  edges = (int*)malloc(sizeof(int)*(amn+1));
  gene = (double*)malloc(sizeof(double)*max*(L+1));
  dna = (double*)malloc(sizeof(double)*max*(L+1));

  switch(bingotype) {
  case 0: decayrate = 0; emptyic = 1; geneexchange = 0; break;
  case 1: decayrate = 10./amn; emptyic = 0; geneexchange = 1; break;
  case 2: decayrate = 50./amn; emptyic = 0; geneexchange = 1; break;
  case 3: decayrate = 0.693/(amn/2); emptyic = 0; geneexchange = 0; min = 0.25; break;
  case 4: decayrate = 0.693/(amn/2); emptyic = 0; geneexchange = 1; min = 0.25; break;
  }
  ...
  for(t = 0; t < amn; t++)
      ...
      for(i = 0; i < max; i++)
	{
	  ...
	  for(k = 0; k < L; k++)
		...
		thisovermin++;
      	      if(gene[i*L+k] > 0)
		gene[i*L+k] -= decayrate*gene[i*L+k];
	      if(dna[i*L+k] == 1)
		gene[i*L+k] = 1;
	      score += gene[i*L+k];
	    }	  
	  bingo[t] += (thisovermin == L ? 1 : 0);
	  total[t] += score;
	}
    }
...
int main(int argc, char *argv[])
{
  ...
  for(i = 2; i < argc; i++)
      ...
      if(!strcmp(argv[i], "--output-trajectories\0"))
	{
	  if(i == argc-1) printf("Number of trajectories to output not specified!");
        ...
        if(!strcmp(argv[i], "--bingo-type\0"))
	{
	  if(i == argc-1) printf("Experiment type not specified!");
	  else
	    {
	      BINGOTYPE = atoi(argv[i+1]);
	      i++;
	    }
	}
        if(!strcmp(argv[i], "--exchange-probability\0"))
	{
	  if(i == argc-1) printf("Exchange probability not specified!");
	  else
	    {
	      exprob = atof(argv[i+1]);
	      i++;
	    }
	}
  ...
  printf("Flags:\n  output-simulation-traces %i\n  remove-singletons %i\n  output-trajectories %i\n  bingo-type %i\n  exchange-probability %.3f\n\n", OUTPUT_SIMULATION_TRACES, REMOVE_SINGLETONS, OUTPUT_TRAJECTORIES, BINGOTYPE, exprob);
  sprintf(filelabel, "%s-%i-%.3f", argv[1], bingotype, exprob);
  ...
  total = (double*)malloc(sizeof(double)*MAXE);
  bingo = (double*)malloc(sizeof(double)*MAXE);
  ...
  if(OUTPUT_TRAJECTORIES > 0)
    {
      sprintf(fstr, "Output/%s-rna3-results-frame.txt", filelabel);
      fp = fopen(fstr, "w");
      fprintf(fp, "L,m,expt,it,n.edges,mean.score,num.bingos,prop.edges,prop.bingos\n");
    }
  
  sprintf(fstr, "Output/%s-rna3-results-overall.txt", filelabel);
  fp2 = fopen(fstr, "w");
  ...
  for(Lindex = 0; Lindex <= 8; Lindex++) 
    {
	  ...
	  if(Lindex == 0)
	    {
	      sprintf(outstr, "Output/%s-rna3-graphs.txt", filelabel);
	      fp1 = fopen(outstr, "w");
	      output_graphs = 1;
	    }
	  ...
	  for(expt = 0; expt <= 37; expt++)
	      ...
	      for(it = 0; it < NIT; it++)
		{
		  ...
		  PlayBingoRNA(amlist, amn, nnodes, L, exprob, bingotype, total, bingo);
		  ...
		  if(it < OUTPUT_TRAJECTORIES)
		    {
		      for(t = 0; t < amn; t+=10)
			{
			  fprintf(fp, "%i,%.3f,%i,%i,%i,%f,%f,%f,%f\n", L, exprob, expt, it, t, (double)total[t]/nnodes, bingo[t], (double)t/amn, (double)bingo[t]/nnodes);
			}
		      fprintf(fp, "\n");
		    }
	      ...
	      fprintf(fp2, "%i,%.3f,%i,", L, exprob, expt);
